    ...
    >
      <div className="grid gap-12 lg:grid-cols-[minmax(18rem,0.7fr)_minmax(0,1.3fr)] lg:gap-20">
        <div>
          ...
          <Link
            href="/faq"
            className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-cyan-200 underline decoration-cyan-300/30 underline-offset-4 transition-colors hover:text-cyan-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"
          >
            Browse the full FAQ
        ...
        <div className="divide-y divide-white/10 border-y border-white/10">
          {faqs.map((faq, index) => (
            <details key={faq.id} className="group">
              <summary className="flex cursor-pointer list-none items-start gap-5 py-6 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-300 [&::-webkit-details-marker]:hidden">
              ...
              <div className="pb-7 pl-[2.6rem] pr-10 sm:pl-[2.75rem]">
                <p className="text-sm leading-7 text-zinc-400 sm:text-base">
                  {faq.answer}
                </p>
                {faq.status === "to-be-announced" ? (
                  <p className="mt-3 font-mono text-[0.6rem] uppercase tracking-[0.16em] text-amber-200/70">
