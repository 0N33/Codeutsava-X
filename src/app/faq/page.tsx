      ...
      >
        <Container>
          ...
          <p className="mt-7 max-w-2xl text-pretty text-base leading-7 text-zinc-300 sm:text-lg sm:leading-8">
            The answers below reflect only currently confirmed information and
            will be updated when registration and participation details go live.
          </p>
        </Container>
      </section>
      ...
      >
        <Container>
          ...
          <div className="mt-12 divide-y divide-white/10 border-y border-white/10">
            {faqs.map((item, index) => (
              <details key={item.id} className="group py-1">
                <summary className="flex cursor-pointer list-none items-start gap-4 rounded-sm px-2 py-6 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 sm:gap-6 sm:px-4">
                ...
                <div className="pb-7 pl-12 pr-4 sm:pl-16">
                  <p className="max-w-3xl text-sm leading-7 text-zinc-400 sm:text-base">
                    {item.answer}
                  </p>
                  {item.status === "to-be-announced" ? (
                    <p className="mt-4 font-mono text-[0.62rem] uppercase tracking-[0.17em] text-zinc-400">
          ...
          <div className="mt-12 border border-white/10 bg-white/[0.025] p-6 sm:flex sm:items-center sm:justify-between sm:gap-8 sm:p-8">
            <div>
              <h2 className="text-xl font-bold text-white">Need another route?</h2>
              <p className="mt-2 text-sm leading-6 text-zinc-400">
                Check the verified community channels while direct inquiries are
                being prepared.
              </p>
            </div>
            <Link
              href="/contact-us"
              className="mt-5 inline-flex min-h-11 items-center justify-center rounded-full border border-cyan-300/40 px-5 py-2.5 text-xs font-bold uppercase tracking-[0.12em] text-cyan-100 hover:bg-cyan-300/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 sm:mt-0"
            >
              Contact options
            </Link>
          </div>
